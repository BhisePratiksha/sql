...
select Brand, Price, FuelType  from Cars
...
select Model, Price, year from Cars
...
select Model, EngineType, Price from Cars
where Price < (select avg(Price) from Cars) and EngineType ='V8'
order by price;

-- 8.	Retrieve brands with more than two car models in the dataset.
select Brand, count(Model) as Total_count from  Cars
group by Brand
...
select Model, FuelType, TransmissionType, Price from Cars
where Price > (select avg(Price) from Cars)
order by Price;


-- 10.	Find the brand with the highest total mileage across all models.
select Brand, sum(Mileage) as Total_model_Mileage from Cars
group by Brand
order by sum(Mileage) limit 1;  
